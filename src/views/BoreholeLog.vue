...
const svg = computed(() => {
  ...
  let s = ''
  // 背景框
  s += `<rect x="${colX.depth}" y="${padTop - 10}" width="${W - colX.depth}" height="${H.value - padTop + 10}" fill="var(--panel)" stroke="var(--border)"/>`
  // 深度刻度
  s += `<rect x="${colX.depth}" y="${padTop - 10}" width="${colX.depthW}" height="${H.value - padTop + 10}" fill="var(--panel-2)"/>`
  for (let d = 0; d <= Math.ceil(maxDepth.value); d += 2) {
    const y = padTop + d * scale
    s += `<line x1="${colX.depth}" y1="${y}" x2="${colX.depth + colX.depthW}" y2="${y}" stroke="var(--border-light)"/>`
    s += `<text x="${colX.depth + colX.depthW - 4}" y="${y + 3}" font-size="10" fill="var(--text-mute)" text-anchor="end">${d}</text>`
  }
  // 分层填色
  layers.forEach((l) => {
  ...
  // 柱状边框
  s += `<rect x="${colX.log}" y="${padTop}" width="${colX.logW}" height="${maxDepth.value * scale}" fill="none" stroke="var(--text-dim)" stroke-width="1"/>`
  // 地下水位线
  ...
  // 右侧分层描述
  s += `<line x1="${colX.desc}" y1="${padTop}" x2="${colX.desc}" y2="${padTop + maxDepth.value * scale}" stroke="var(--border)"/>`
  layers.forEach((l) => {
    s += `<text x="${colX.desc + 6}" y="${(l.y1 + l.y2) / 2 - 2}" font-size="11" fill="var(--text)">${l.name}</text>`
    s += `<text x="${colX.desc + 6}" y="${(l.y1 + l.y2) / 2 + 12}" font-size="10" fill="var(--text-mute)">${l.top.toFixed(1)}–${l.bottom.toFixed(1)}m  厚${(l.bottom - l.top).toFixed(1)}m</text>`
  })
  // 标题
  s += `<text x="${W / 2}" y="16" font-size="14" fill="var(--text)" text-anchor="middle" font-weight="600">${props.code} 钻孔柱状图</text>`
  return s
})
</script>
