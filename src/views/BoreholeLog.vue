...
import { boreholeLayers, waterLevel } from '../data/treeData'
...
const padTop = 30
const padBottom = 20
const maxDepth = boreholeLayers[boreholeLayers.length - 1].bottom
const H = padTop + maxDepth * scale + padBottom

const colX = { depth: 0, depthW: 56, log: 56, logW: 220, desc: 276 }

const svg = computed(() => {
  const layers = boreholeLayers.map((l) => {
  ...
  let s = ''
  // 背景框
  s += `<rect x="${colX.depth}" y="${padTop - 10}" width="${W - colX.depth}" height="${H - padTop + 10}" fill="#1f1f38" stroke="#34344f"/>`
  // 深度刻度
  s += `<rect x="${colX.depth}" y="${padTop - 10}" width="${colX.depthW}" height="${H - padTop + 10}" fill="#252542"/>`
  for (let d = 0; d <= Math.ceil(maxDepth); d += 2) {
  ...
  // 柱状边框
  s += `<rect x="${colX.log}" y="${padTop}" width="${colX.logW}" height="${maxDepth * scale}" fill="none" stroke="#d7dbe2" stroke-width="1"/>`
  // 地下水位线
  ...
  // 右侧分层描述
  s += `<line x1="${colX.desc}" y1="${padTop}" x2="${colX.desc}" y2="${padTop + maxDepth * scale}" stroke="#34344f"/>`
  layers.forEach((l) => {
